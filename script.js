...
if (btnSubmit) {
  btnSubmit.addEventListener('click', () => {
    ...
    const msg =
      ...
      `*E-mail:* ${email}\n\n` +
      `*Projeto:*\n${detalhes}`;
    window.open(`https://wa.me/558486628135?text=${encodeURIComponent(msg)}`, '_blank');
  });
}
...
(function () {
  ...
  /* Prévia silenciosa toca automaticamente assim que possível (PC e Mobile),
     sem precisar clicar — o som só é liberado ao abrir o vídeo no modal.
     (autoplay desativado se o usuário/sistema pedir "reduzir movimento") */
  if (!reduceMotion) {
    cards.forEach(card => {
      const vid = card.querySelector('.vid-preview');
      if (!vid) return;
      vid.muted = true;
  ...
  /* Observer mantém a reprodução ativa quando o card entra na tela e
     pausa quando sai (economiza bateria/dados), sem exigir clique */
